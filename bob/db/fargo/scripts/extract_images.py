    ...
    for session in sessions: 
      ...
      for condition in ['SR300-laptop', 'SR300-mobile']:
        ...
        for recording in ['0', '1']:
          ...
          color_timestamps = load_timestamps(os.path.join(session_dir, condition, recording, 'streams', 'color_timestamps.txt'))
          ir_timestamps = load_timestamps(os.path.join(session_dir, condition, recording, 'streams', 'ir_timestamps.txt'))
          depth_timestamps = load_timestamps(os.path.join(session_dir, condition, recording, 'streams', 'depth_timestamps.txt'))
          
          # get the index of the first annotated frame in the stream
          first_annotated_frame_indices, status = get_first_annotated_frame_index(os.path.join(session_dir, condition, recording, 'annotations', 'color_timestamps.txt'))
          if status < 0:
          ...
          annotation_dir = os.path.join(session_dir, condition, recording, 'annotations')
          ...
          for i, frame in enumerate(color_stream):
            ...
            if toto % 4 == 0 and i < last_frame_index:
              ...
              if bool(args['--plot']):
                ...
                axarr[0].imshow(to_save, cmap='gray')
